...
class ComandaDB(db.Base):
    ...
    cliente_id = Column(Integer, ForeignKey('tb_cliente.id'), nullable=True, default=None)
    def __init__(self, id, comanda, data_hora, status, funcionario_id, cliente_id):
        self.id = id
...
class ComandaProdutoDB(db.Base):
    ...
    def __init__(self, id, comanda_id, produto_id, funcionario_id, quantidade, valor_unitario):
        self.id = id
