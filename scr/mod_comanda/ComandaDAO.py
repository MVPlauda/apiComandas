...
@router.get("/comanda/{id}", tags=["Comanda"])
def get_comanda(id: int):
    try:
        session = db.Session()
        # busca filtrando pelo status
        aux_dados = session.query(ComandaDB, FuncionarioDB, ClienteDB)\
            .select_from(ComandaDB)\
            .join(FuncionarioDB, FuncionarioDB.id == ComandaDB.funcionario_id, isouter=False)\
            .join(ClienteDB, ClienteDB.id == ComandaDB.cliente_id, isouter=True)\
            .filter(ComandaDB.id == id)\
            .order_by(ComandaDB.id)\
            .all()
...
def put_comanda(corpo: Comanda):
    try:
        ...
        dados = session.query(ComandaDB).filter(ComandaDB.id == corpo.id).one()
        # atualiza os dados
        #dados.id == corpo.id
...
def get_comanda_item(comanda_id: int):
    try:
        ...
        .join(FuncionarioDB, FuncionarioDB.id == ComandaProdutoDB.funcionario_id, isouter=False)\
        .join(ProdutoDB, ProdutoDB.id == ComandaProdutoDB.produto_id, isouter=False)\
...
def put_comanda_item(corpo: ComandaProdutos):
    try:
        ...
        dados = session.query(ComandaProdutoDB).filter(ComandaProdutoDB.id == corpo.id).one()
